...
import (
	...
	// "openmcpscheduler/pkg/controller/framework/plugins/predicates"
		...
		for rName, rNeeded := range container.Resources.Needs {
			...
			if !exist {
				...
				if val == false && rNeeded == true {
					...
					// klog.Infof("[IsNeedResourceMap] %v, %v", rName, rNeeded)
				}
			}
		}
...
// Scheduling tries to schedule the given OpenMCPDeployment to one of the clsuter
// If it succeeds, it will return the map of the cluster
func(cm *ClusterManager) Scheduling (dep *ketiv1alpha1.OpenMCPDeployment) map[string]int32 {
	...
	// Create OpenMCPScheduler
	// Have to change framework implementation....
	...
	// // Get data from influxdb for testing
	// // getResources()
	...
	// klog.Infof("Getting Infromation from New Pod, %v", newPod)
...
// Schedule one replica of openmcpDeployment
