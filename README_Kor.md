...
```
> PowerDNS 서버 IP가 퍼블릭이 아닌 경우 외부 클러스터에서 접근이 불가하므로 openmcp/install_openmcp/member.back/configmap/coredns/coredns-cm.yaml 파일에서 변수 값을 포트포워딩한 IP로 수정한 후 외부 클러스터에 배포한다.
