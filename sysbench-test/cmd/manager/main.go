...
var START_CPU_CMD_START string = "sysbench-cpu --test=cpu --cpu-max-prime=1000 --num-threads=" // 1,2,3,4
var START_CPU_CMD_END string = " run"
var START_MEMORY_CMD_START string = "sysbench-memory --test=memory --memory-block-size="
var START_MEMORY_CMD_END string = " --memory-total-size=100G run"
...
func (h *HttpManager) memory_stop(w http.ResponseWriter, r *http.Request) {
	...
	if h.MemoryBenchStatus == "START" {
		...
		cmdStr := "killall -9 sysbench-memory"
		fmt.Println(cmdStr)
		_, err := util.CmdExec(cmdStr)
		if err != nil {
			fmt.Println("Err !", err)
...
func (h *HttpManager) disk_stop(w http.ResponseWriter, r *http.Request) {
	...
	if h.DiskBenchStatus == "START" {
		...
		cmdStr := "killall -9 sysbench-disk"
		fmt.Println(cmdStr)
		_, err := util.CmdExec(cmdStr)
		if err != nil {
			fmt.Println("Err !", err)
...
func main() {
	...
	handler.HandleFunc("/status", httpManager.printStatus)
