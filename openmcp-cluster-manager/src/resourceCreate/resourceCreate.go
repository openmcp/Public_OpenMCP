...
func createSubResourceDeployment(clusterName string, cm *clusterManager.ClusterManager) error {
	//omcplog.V(2).Info("[Create Resource]'" + clusterName + "' Start")
	//omcplog.V(2).Info("[Create Resource] Deployment Resource by OpenMCP")
	...
	for _, odep := range odepList.Items {
		...
		omcplog.V(2).Info("[Create Resource] Found Deployment '" + odep.Name + "' in Cluster. Update Status Scheduling Need='true' & Complete='false'")
