...
HBASE = os.environ["HMASTER"] #"hbase-master-svc.datacenter.svc.cluster.local"
...
def saveHBaseData(data_dict) :
  connection = happybase.Connection(INFLUXDB, autoconnect=False)
  connection.open()
  ...
  Time = data_dict['Time']
  CraneFullName = data_dict['CraneFullName']
  table = connection.table(CraneFullName + data_table)

  dict_put = {}
  
  for key, value in data_dict.items() :
    dict_put['Value:' + key] = str(value)
  
  table.put(Time, dict_put)


  #################### Format Error Noe ####################
  table = connection.table(CraneFullName + version_table)

  dict_put = {}
  dict_put["Version:FormatError"] = "None"
  
  table.put("FormatError", dict_put)

  connection.close()

def saveHBaseFormatError(error_dict):
  # error_dict : "FormatError" = Crane or OBD
  # error_dict : "CraneFullName"
  connection = happybase.Connection(HBASE, autoconnect=False)
  connection.open()


  CraneFullName = error_dict['CraneFullName']
  FormatError = error_dict['FormatError']

  table = connection.table(CraneFullName + version_table)

  dict_put = {}
  dict_put["Version:FormatError"] = FormatError
...
def msg_parse(data) :
  ...
  OBD_II_data_dict = {}
  sensor_data_dict = {}
  # OBD를 사용하지 않는 경우 (Null 인경우)
  # 파싱하지 않음
  #if len(OBD_DataList) == 0 and OBD_DataList[0] == None:
  #  pass

  #else:
  #  # OBD Data 유효성 체크
  #  OBD_vailable = OBD_isVailable(OBD_DataList)

   # if not OBD_vailable :
   #   error_dict = {}
   #   error_dict["FormatError"] = "OBD"
   #   error_dict["CraneFullName"] = CraneFullName
   #   jsonString = makeJson(error_dict)
   #   return jsonString

   # PID_COUNT = get_pid_count()
   # if PID_COUNT != 0 :
   #   print("--------OBD-II Massage Parsing--------")
   #   for i in range(PID_COUNT):
   #     description, pid,A,B,C,D = OBD_II_Parser(OBD_DataList[i])
   #     OBD_II_data_dict[description] = str(OBD_II_Calc(pid,A,B,C,D))
  
  # Crane를 사용하지 않는 경우 (Null 인경우)
  # 파싱하지 않음
  if Crane_Data == None:
    pass
...
def transfer_list(json_list) :
  ...
  for json_data in json_list:
    ...
    if not "FormatError" in check_dict:
       ...
       print("ERROR")
#      saveHBaseFormatError(check_dict)
