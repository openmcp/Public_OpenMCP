...
let apiParams = "";
class CsStorageClass extends Component {
  constructor(props) {
    super(props);
    this.state = {
      columns: [
        { name: "name", title: "Name" },
        { name: "provisioner", title: "Provisioner" },
        { name: "volume_count", title: "Volume Count" },
        { name: "default", title: "Default" },
      ],
      defaultColumnWidths: [
        { columnName: "name", width: 130 },
        { columnName: "provisioner", width: 280 },
        { columnName: "volume_count", width: 180 },
        { columnName: "default", width: 130 },
      ],
      rows: "",
  ...
  callApi = async () => {
    ...
    const response = await fetch(`/clusters/${apiParams}/storage_class`);
    ...
    const Cell = (props) => {
      ...
      if (column.name === "status") {
        ...
        return (
          ...
          ><Link to={{
            pathname: `/clusters/${apiParams}/storage_class/${props.value}`,
            state: {
              data : row
            }
