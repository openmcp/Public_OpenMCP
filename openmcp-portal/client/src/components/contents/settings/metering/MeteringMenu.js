...
class MeteringMenu extends Component {
  state = {
    // rows: "",
    // completed: 0,
    reRender: "",
    value: 0,
    tabHeader: [
      { label: "metering", index: 1, param:"metering" },
      { label: "bill", index: 2, param:"bill" },
  ...
  componentWillMount() {
     ...
     if(this.props.match.url.indexOf("bill") > 0 ){
            ...
            <div className={classes.root}>
                ...
                >
                  {this.state.tabHeader.map((i) => {
                    ...
                    <Tab label={i.label} {...a11yProps(i.index)}
                          component={Link}
                          to={{
                            pathname: `/settings/meterings/${i.param}`
                          }}
              ...
              <TabPanel className="tab-panel" value={this.state.value} index={0}>
                ...
                <Route path="/settings/meterings/metering"
                    render={({match,location}) => <Metering  match={match} location={location} menuData={this.onMenuData}/>} >
                  </Route>
                </Switch>
              </TabPanel>
              <TabPanel className="tab-panel" value={this.state.value} index={1}>
               <Switch>
                  <Route path="/settings/meterings/bill"
                    render={({match,location}) => <BillList  match={match} location={location} menuData={this.onMenuData}/>} >
                  </Route>
                </Switch>
              </TabPanel>
            </div>
